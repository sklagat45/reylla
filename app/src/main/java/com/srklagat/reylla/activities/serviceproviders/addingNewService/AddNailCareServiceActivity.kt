...
import kotlinx.android.synthetic.main.activity_add_nail_care_service.*
import java.io.IOException

class AddNailCareServiceActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_add_nail_care_service)

        setupActionBar()

        // Assign the click event to iv_add_update_product image.
        tv_add_nail_service_image.setOnClickListener {
        ...
        btn_save_nail_service.setOnClickListener {
                        ...
                        .addOnSuccessListener { taskSnapshot ->
                            ...
                            taskSnapshot.metadata!!.reference!!.downloadUrl
                                .addOnSuccessListener { url ->
                                    ...
                                    Toast.makeText(
                                        this@AddNailCareServiceActivity,
                                    ...
                                    // START
                                    Glide.with(this@AddNailCareServiceActivity)
                                        .load(url)
                                        .placeholder(R.drawable.nail_care) // If the image is failed to load then the placeholder will be visible.
                                        .into(iv_nail_service_image)
                                    // END
                                }
...
//                        iv_place_image.setImageURI(mSelectedImageFileUri)
    ...
    private fun setupActionBar() {
        ...
        setSupportActionBar(toolbar_add_nail_care_activity)
        ...
        toolbar_add_nail_care_activity.setNavigationOnClickListener { onBackPressed() }
    }
    ...
    private fun validateServiceDetails(): Boolean {
        return when {
            ...
            TextUtils.isEmpty(et_nail_style_name.text.toString().trim { it <= ' ' }) -> {
                showErrorSnackBar("Enter the style name")
                false
            }

            TextUtils.isEmpty(et_nail_service_duration.text.toString().trim { it <= ' ' }) -> {
                showErrorSnackBar("Enter styling Duration")
                false
            }

            TextUtils.isEmpty(et_nail_style_cost.text.toString().trim { it <= ' ' }) -> {
    ...
    private fun uploadProductDetails() {
        ...
        val nailService = NailService(
            providerID,
            et_nail_style_name.text.toString().trim { it <= ' ' },
            et_nail_service_duration.text.toString().trim { it <= ' ' },
            et_nail_style_cost.text.toString().trim { it <= ' ' },
            mserviceImageURL
        )

        FirestoreClass().uploadNailDetails(this@AddNailCareServiceActivity, nailService)
    }
...
//        hideProgressDialog()
        ...
        Toast.makeText(
            this@AddNailCareServiceActivity, "data uploaded successfully",
            Toast.LENGTH_SHORT
        ).show()
