package com.srklagat.reylla.activities.serviceproviders.addingNewService
...
import kotlinx.android.synthetic.main.activity_add_masage_service.*
import java.io.IOException

class AddMassageServiceActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_add_masage_service)

        setupActionBar()

        // Assign the click event to iv_add_update_product image.
        tv_add_service_image.setOnClickListener {
        ...
        btn_save_massage_service.setOnClickListener {
                        ...
                        .addOnSuccessListener { taskSnapshot ->
                            ...
                            taskSnapshot.metadata!!.reference!!.downloadUrl
                                .addOnSuccessListener { url ->
                                    ...
                                    Toast.makeText(
                                        this@AddMassageServiceActivity,
                                    ...
                                    // START
                                    Glide.with(this@AddMassageServiceActivity)
                                        .load(url)
                                        .placeholder(R.drawable.massage_img) // If the image is failed to load then the placeholder will be visible.
                                        .into(iv_massage_service_image)
                                    // END
                                }
...
//                        iv_place_image.setImageURI(mSelectedImageFileUri)
    ...
    private fun setupActionBar() {
        ...
        setSupportActionBar(toolbar_add_massage_care_activity)
        ...
        toolbar_add_massage_care_activity.setNavigationOnClickListener { onBackPressed() }
    }
    ...
    private fun validateServiceDetails(): Boolean {
        return when {
            ...
            TextUtils.isEmpty(et_massage_style_name.text.toString().trim { it <= ' ' }) -> {
                showErrorSnackBar("Enter the style name")
                false
            }

            TextUtils.isEmpty(et_massage_service_duration.text.toString().trim { it <= ' ' }) -> {
                showErrorSnackBar("Enter styling Duration")
                false
            }

            TextUtils.isEmpty(et_massage_service_cost.text.toString().trim { it <= ' ' }) -> {
    ...
    private fun uploadProductDetails() {
        ...
        val massageService = MassageService(
            providerID,
            et_massage_style_name.text.toString().trim { it <= ' ' },
            et_massage_service_duration.text.toString().trim { it <= ' ' },
            et_massage_service_cost.text.toString().trim { it <= ' ' },
            mserviceImageURL
        )

        FirestoreClass().uploadMassageDetails(this@AddMassageServiceActivity, massageService)
    }
...
//        hideProgressDialog()
        ...
        Toast.makeText(
            this@AddMassageServiceActivity, "data uploaded successfully",
            Toast.LENGTH_SHORT
        ).show()
