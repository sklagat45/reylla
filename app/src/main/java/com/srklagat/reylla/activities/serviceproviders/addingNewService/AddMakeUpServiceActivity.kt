...
import com.srklagat.reylla.model.MakeupService
import kotlinx.android.synthetic.main.activity_add_make_up_service.*
import java.io.IOException

class AddMakeUpServiceActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_add_make_up_service)

        setupActionBar()

        // Assign the click event to iv_add_update_product image.
        tv_add_makeup_service_image.setOnClickListener {
        ...
        btn_save_makeup_service.setOnClickListener {
                        ...
                        .addOnSuccessListener { taskSnapshot ->
                            ...
                            taskSnapshot.metadata!!.reference!!.downloadUrl
                                .addOnSuccessListener { url ->
                                    ...
                                    // START
                                    Glide.with(this@AddMakeUpServiceActivity)
                                        .load(url)
                                        .placeholder(R.drawable.makeup_img1) // If the image is failed to load then the placeholder will be visible.
...
//                        iv_place_image.setImageURI(mSelectedImageFileUri)
    ...
    private fun setupActionBar() {
        ...
        setSupportActionBar(toolbar_add_hair_care_activity)
        ...
        toolbar_add_hair_care_activity.setNavigationOnClickListener { onBackPressed() }
    }
    ...
    private fun validateServiceDetails(): Boolean {
        return when {
            ...
            TextUtils.isEmpty(et_make_up_name.text.toString().trim { it <= ' ' }) -> {
                showErrorSnackBar("Enter the style name")
                false
            }

            TextUtils.isEmpty(et_make_up_service_duration.text.toString().trim { it <= ' ' }) -> {
                showErrorSnackBar("Enter styling Duration")
                false
            }

            TextUtils.isEmpty(et_make_up_style_cost.text.toString().trim { it <= ' ' }) -> {
    ...
    private fun uploadProductDetails() {
        ...
        val makeupService = MakeupService(
            providerID,
...
//        hideProgressDialog()
        ...
        Toast.makeText(
            this@AddMakeUpServiceActivity, "data uploaded successfully",
            Toast.LENGTH_SHORT
        ).show()
