...
import kotlinx.android.synthetic.main.activity_add_tatoo_and_colour_service.*
import java.io.IOException

class AddTatooAndColourServiceActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_add_tatoo_and_colour_service)

        setupActionBar()

        // Assign the click event to iv_add_update_product image.
        tv_add_tat_service_image.setOnClickListener {
        ...
        btn_save_design_style_service.setOnClickListener {
                        ...
                        .addOnSuccessListener { taskSnapshot ->
                            ...
                            taskSnapshot.metadata!!.reference!!.downloadUrl
                                .addOnSuccessListener { url ->
                                    ...
                                    Toast.makeText(
                                        this@AddTatooAndColourServiceActivity,
                                    ...
                                    // START
                                    Glide.with(this@AddTatooAndColourServiceActivity)
                                        .load(url)
                                        .placeholder(R.drawable.angel_tattoo) // If the image is failed to load then the placeholder will be visible.
...
//                        iv_place_image.setImageURI(mSelectedImageFileUri)
    ...
    private fun setupActionBar() {
        ...
        setSupportActionBar(toolbar_add_tat_color_activity)
        ...
        toolbar_add_tat_color_activity.setNavigationOnClickListener { onBackPressed() }
    }
    ...
    private fun validateServiceDetails(): Boolean {
        return when {
            ...
            TextUtils.isEmpty(et_design_name.text.toString().trim { it <= ' ' }) -> {
                showErrorSnackBar("Enter the style name")
                false
            }

            TextUtils.isEmpty(et_design_service_duration.text.toString().trim { it <= ' ' }) -> {
                showErrorSnackBar("Enter styling Duration")
                false
            }

            TextUtils.isEmpty(et_tat_style_cost.text.toString().trim { it <= ' ' }) -> {
    ...
    private fun uploadProductDetails() {
        ...
        val tatColorService = TatColorService(
            providerID,
            et_design_name.text.toString().trim { it <= ' ' },
            et_design_service_duration.text.toString().trim { it <= ' ' },
            et_tat_style_cost.text.toString().trim { it <= ' ' },
            mserviceImageURL
        )

        FirestoreClass().uploadTatColourDetails(
            this@AddTatooAndColourServiceActivity,
            tatColorService
        )
    }
...
//        hideProgressDialog()
        ...
        Toast.makeText(
            this@AddTatooAndColourServiceActivity, "data uploaded successfully",
            Toast.LENGTH_SHORT
        ).show()
